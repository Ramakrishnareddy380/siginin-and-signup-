pipeline {
    ...
    stages {
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    sh '/usr/local/bin/docker-credential-desktop --version'
                    sh '/usr/local/bin/docker-credential-desktop build -t app .'
                    sh '/usr/local/bin/docker-credential-desktop push app'
                    }
                }
            }
        stage('Build') {
